...
def find_py_gd_library(shared_lib):
    return os.path.join(imp.find_module('py_gd')[1], shared_lib)
...
def main(argv):
    ...
    # find the path to libgd
    # Note: Should we implement a list of libraries that we can iterate over?
    lib_name = 'libgd.3.dylib'
    lib_path = get_conda_lib_path(lib_name)
    py_gd_library = find_py_gd_library('py_gd.so')

    #command = ('install_name_tool -change {} {} {}'
    #           .format(lib_name, lib_path, py_gd_library))
